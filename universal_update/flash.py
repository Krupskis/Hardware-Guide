...
if len(sys.argv) < 2:
    ...
    print(f"{WARNING}You can find the port by running 'ls /dev/tty.*' on macOS, 'ls /dev/tty*' on Linux or on Windows by opening Device Manager.{RESET}")
    print(f"{WARNING}Mac example: python flash.py /dev/tty.usbmodem14101{RESET}")
    print(f"{WARNING}Linux example: python flash.py /dev/ttyACM0{RESET}")
    print(f"{WARNING}Windows example: python flash.py COM3{RESET}")
    sys.exit(1)
